...
<%
  ...
  //修改锚点，根据所处界面动态调整默认文档类型
  String FatherType;
  try{
	FatherType=MqlUtil.mqlCommand(context, "print bus "+objectId+" select type dump;");
		 if (FatherType.equals("Workspace Vault")){
			 documentType="type_BP_RequireDocument";
		 }
	} catch (Exception e)
      {
      }
...
%>
  function showTypeSelector()
  {
    ...
    var strURL="../common/emxTypeChooser.jsp?fieldNameDisplay=type&fieldNameActual=realType&formName=frmMain&ShowIcons=true&InclusionList=type_Document&ExclusionList=<%=excludeTypes%>&ObserveHidden=true&SelectType=singleselect&ReloadOpener=true";
...
<%
     ...
     {//修改锚点，将自动命名的输入类型改为不可见checkbox转hidden同时删除了名字
%>
        ...
        <input type="hidden" name="AutoName" value="<xss:encodeForHTMLAttribute><%=documentAutoName%></xss:encodeForHTMLAttribute>" onClick="txtNameFocus()"  />&nbsp;
...
<%
    ...
    if ( showTitle.equalsIgnoreCase("true") || showTitle.equalsIgnoreCase("required") )
    {
        ...
        {
            txtLable = "labelRequired";
			//修改锚点隐藏标题行，将type从text改为hidden删除了“标题”字符 <emxUtil:i18n localize="i18nId">emxComponents.Common.Title</emxUtil:i18n>
        }
%>
    <tr><!--//XSSOK-->
      <td class="<%=txtLable%>" >
      ...
      <td class="inputField" >
        <input type="hidden" name="title" size="20" value="<xss:encodeForHTMLAttribute><%=UIUtil.isNullOrEmpty(documentTitle) ? (UIUtil.isNullOrEmpty(defaultTitle) ? "" : defaultTitle):documentTitle%></xss:encodeForHTMLAttribute>" />
      </td>
    </tr>
...
//added third parameter to get the multiline value for bug no 338579
  ...
  System.out.println("excludeAttrList==="+excludeAttrList);
  //注释锚点，根据拓展的属性动态生成属性列
  while(i.hasNext())
  {
    Map attributeMap = (Map)i.next();
    attributeName = (String)attributeMap.get("name");
	//锚点 add by lilei 去除cadtype 20240429
	if("CAD Type".equals(attributeName)){
		continue;
	}
	//锚点 add by lilei 去除cadtype 20240429
