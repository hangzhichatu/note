...
<%	
	...
	try{
	String selectDocumentId=request.getParameter("stringListParam");//传入所选行的id（文档对象）
	String redirectToURL = "../common/emxIndentedTable.jsp?program=emxLibraryCentralUtil:getAllLibraries&table=TMCSelectFolder&expandProgram=BP_GetSubClass:BPgetSubclasses&header=\u9009\u62e9\u5e93&cancelLabel=\u53d6\u6d88&submitURL=../teamcentral/BP_AddDocumentToClass.jsp&submitLabel=\u63d0\u4ea4&stringListParam=" + selectDocumentId;
	//String selectDocumentId=(String) session.getAttribute("BPDocumentRowsId");
	
    String[] emxTableRowIds=request.getParameterValues("emxTableRowId");
	if(selectDocumentId.equals("")){
		selectDocumentId=(String) session.getAttribute("BPDocumentRowsId");
	}
	String[] sourceObjects = FrameworkUtil.getSplitTableRowIds(emxTableRowIds);//该字符串的0位是所选的库的分类的id
	...
	programMap.put("selectDocumentId",selectDocumentId);
	programMap.put("SelectClass",selectclassId);
	...
	Map retMap=JPO.invoke(context, "BP_AddDocumentToClass",null, "CreatRelationshipFromClassToDocument", JPO.packArgs(programMap),Map.class);
