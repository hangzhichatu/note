...
@Path("/info")
public class CHIDIDataExchangeServiceBase extends RestService {
	...
	* @date: 2024年4月23日 下午10:30:00
	*/
	@POST // 调用方式
	@Consumes({ MediaType.APPLICATION_JSON }) // 输入格式
	@Produces({ MediaType.APPLICATION_JSON }) // 输出格式
	@Path("/callJPO")
	public Response callJPO(@Context HttpServletRequest request, @RequestBody String requestJson) throws Exception {
		System.out.println("callJPO.." + requestJson);
...
//		System.out.println("fileId=" + fileId);
		...
		respJsonObj.add("details", "callJPO");
	...
	@Produces({ MediaType.APPLICATION_JSON }) // 输出格式
	@Path("/excuteMQL")
	public Response excuteMQL(@Context HttpServletRequest request, @RequestBody String requestJson) throws Exception {
		System.out.println("excuteMQL.." + requestJson);
		matrix.db.Context context = null;
		String message = "";
		try {
		context = new matrix.db.Context("https://plmtest.longi.com/internal");
		context.setUser("admin_platform");
		context.setPassword("Aa123456");
		context.connect();
		JSONObject reqJsonObj = JSONObject.parseObject(requestJson);
		String mqlCommand = reqJsonObj.getString("mql");
		message = MqlUtil.mqlCommand(context, false, mqlCommand, true);
...
//		System.out.println("fileId=" + fileId);
		...
		JsonObjectBuilder respJsonObj = Json.createObjectBuilder();
		respJsonObj.add("code", 200);
		respJsonObj.add("message", message);
		respJsonObj.add("details", "excuteMQL");
		return Response.status(200).entity(respJsonObj.build().toString()).build();
	}
	/*
