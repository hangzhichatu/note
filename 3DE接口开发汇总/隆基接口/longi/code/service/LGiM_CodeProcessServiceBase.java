...
public class LGiM_CodeProcessServiceBase extends RestService {
    ...
    public Response updateManagementProcessMessage(@Context HttpServletRequest request, @RequestBody String requestJson) throws Exception {
        ...
        interfaceLogger.info("BPM外购物料表单内容更新请求入参：" + requestJson);
        ...
        try {
            ...
            respJsonObj = (JsonObjectBuilder) JPO.invoke(context, "LGiM_CodeApplicationForm", null, "updateProcess", JPO.packArgs(argsMap), JsonObjectBuilder.class);
            System.out.println("updateManagementProcessMessage..end : " + respJsonObj);
            interfaceLogger.info("BPM外购物料表单内容更新请求返回：" + respJsonObj.toString());
            return Response.status(200).entity(respJsonObj.build().toString()).build();
        } catch (Exception e) {
            e.printStackTrace();
            respJsonObj.add("msg", "PLM_Code外购物料表单流程更新接口异常：" + e);
            interfaceLogger.info("BPM外购物料表单内容更新请求失败返回：" + respJsonObj.toString());
